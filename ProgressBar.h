...
struct ProgressBar : public ScreenElement
{
	...
	void draw(TextureManager* texture_manager)
	{
		...
		ScreenElement bar(x1, y1, (x2 - x1) * progress + x1, y2, "green.png");
		bar.draw(texture_manager);
		ScreenElement white(x1, y1, (x2 - x1) * progress + x1, y2, "white.png");
		glColor4f(1.0, 1.0, 1.0, white_alpha);
		white.draw(texture_manager);
		glColor4f(1.0, 1.0, 1.0, 1.0);
		if (!no_text)
		{
			draw_string(texture_manager, 32, "Now downloading...", x1, y2 + 100);
			draw_string(texture_manager, 32, "Time remaining: " + to_string((int)floor((float)frames_left / 60.0)) + " seconds", x1, y2 + 50);
		}
	}
